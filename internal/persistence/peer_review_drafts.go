...
func (s *Store) LoadPeerReviewDraft(ctx context.Context, batchID, draftID string) (*domain.PeerReviewDraft, error) {
	return scanDraft(s.db.QueryRowContext(ctx, `SELECT document FROM peer_review_drafts WHERE id=? AND batch_id=?`, draftID, batchID))
}

func (t *Tx) LoadPeerReviewDraft(ctx context.Context, batchID, draftID string) (*domain.PeerReviewDraft, error) {
	return scanDraft(t.tx.QueryRowContext(ctx, `SELECT document FROM peer_review_drafts WHERE id=? AND batch_id=?`, draftID, batchID))
}
