...
type Store struct {
	db                  *sql.DB
	auditStatementMu    sync.Mutex
	auditStatementCache map[string]*cachedAuditStatement
}
type Tx struct{ tx *sql.Tx }

type cachedAuditStatement struct {
	connection *sql.Conn
	statement  *sql.Stmt
}
...
func Open(path string) (*Store, error) {
	...
	store := &Store{db: db, auditStatementCache: map[string]*cachedAuditStatement{}}
