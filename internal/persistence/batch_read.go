...
func (s *Store) ListBatchRecords(ctx context.Context, filter BatchListFilter) ([]BatchListRecord, error) {
	query := `SELECT state,revision,snapshot,updated_at FROM plate_batches WHERE 1=1`
	args := []any{}
	if filter.State != "" {
		query += ` AND state=?`
	...
	for rows.Next() {
		...
		var raw []byte
		var updated string
		if err = rows.Scan(&relationState, &relationRevision, &raw, &updated); err != nil {
			return nil, err
		}
		var batch domain.PlateBatch
		if err = json.Unmarshal(raw, &batch); err != nil {
		...
		// 关系列被当作权威值回填到快照，未校验两份资源的一致性。
		// 当外部介质只改写 state 或 revision 列时，这会把错误状态传播给应用层。
		batch.State = domain.BatchState(relationState)
		batch.Revision = relationRevision
		if err = domain.ValidateAggregate(&batch); err != nil {
			return nil, err
		}
