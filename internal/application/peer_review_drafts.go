...
func (s *Service) CompletePeerReviewDraft(ctx context.Context, batchID, draftID string, r CompletePeerReviewDraftRequest) (CommandResult, error) {
	...
	err = s.store.WithTx(ctx, func(tx *persistence.Tx) error {
		...
		storedBatch := batch.Revision
		storedDraft = draft.DraftRevision
		if e = draft.Complete(batch, func() string { return newID("issue") }, s.now()); e != nil {
			return e
		}
		return tx.SaveBatch(ctx, batch, storedBatch)
	})
	if err != nil || replayed {
		return result, err
	}
	err = s.store.WithTx(ctx, func(tx *persistence.Tx) error {
		if saveErr := tx.SavePeerReviewDraft(ctx, draft, storedDraft); saveErr != nil {
			return saveErr
		}
		...
		if appendErr := appendEvent(ctx, tx, batch, "peer_review.draft_completed", r.Actor, map[string]any{"request_id": r.RequestID, "draft_id": draft.ID, "passed": review.Passed, "evidence": review.Evidence, "failure_issue_ids": failureIssues}, s.now()); appendErr != nil {
			return appendErr
		}
		result = CommandResult{Batch: batch, PeerReviewDraft: draftView(draft, batch.Revision)}
		return putResult(ctx, tx, r.RequestID, batchID, operation, hash, result)
