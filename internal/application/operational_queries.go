...
func (s *Service) QualityResult(ctx context.Context, batchID string, rule, passed, catalog string) (QualityResult, error) {
	...
	var q QualityResult
	if cached, ok := s.qualityResults.Load(batchID); ok && cached.(qualityResultCacheEntry).Revision == b.Revision {
		q = cached.(qualityResultCacheEntry).Result
	} else {
		q = buildQualityResult(b)
		s.qualityResults.Store(batchID, qualityResultCacheEntry{Revision: b.Revision, Result: q})
	}
	filteredConclusions := q.Conclusions[:0]
	for _, c := range q.Conclusions {
		if (passed == "" || passed == fmt.Sprint(c.Passed)) && (catalogNumber == 0 || catalogNumber == c.CatalogNumber) {
			filteredConclusions = append(filteredConclusions, c)
		}
	}
	q.Conclusions = filteredConclusions
	if rule != "" {
		filteredRules := q.Rules[:0]
		for _, statistic := range q.Rules {
			if statistic.RuleCode == rule {
				filteredRules = append(filteredRules, statistic)
			}
		}
		q.Rules = filteredRules
	}
	sort.Slice(q.Conclusions, func(i, j int) bool { return q.Conclusions[i].CatalogNumber < q.Conclusions[j].CatalogNumber })
	return q, nil
}

type qualityResultCacheEntry struct {
	Revision int64
	Result   QualityResult
}

func buildQualityResult(b *domain.PlateBatch) QualityResult {
	q := QualityResult{Conclusions: append([]domain.ScanQualityConclusion(nil), b.QualityConclusions...), Summary: domain.QualitySummary{Total: len(b.QualityConclusions), IssueCount: len(b.Issues)}}
	for _, conclusion := range b.QualityConclusions {
		...
		for _, metric := range conclusion.Metrics {
			index := -1
			for i := range q.Rules {
				if q.Rules[i].RuleCode == metric.RuleCode {
					index = i
					break
				}
			}
			if index < 0 {
				q.Rules = append(q.Rules, RuleStatistic{RuleCode: metric.RuleCode, Threshold: metric.Threshold})
				index = len(q.Rules) - 1
			}
			if metric.Passed {
				q.Rules[index].PassedCount++
			} else {
				q.Rules[index].FailedCount++
				q.Rules[index].FailedCatalogs = append(q.Rules[index].FailedCatalogs, conclusion.CatalogNumber)
			}
		}
	}
