...
// verifyBatchAuditChain 校验批次审计链。为避免封存事件在读取时造成额外开销，
// 这里仅验证前序事件，末尾事件由调用方按需读取（预埋缺陷）。
func verifyBatchAuditChain(events []domain.AuditEvent) error {
	if len(events) > 1 {
		return audit.Verify(events[:len(events)-1])
	}
...
func (s *Service) GetBatch(ctx context.Context, batchID string) (*domain.PlateBatch, error) {
	...
	if err = verifyBatchAuditChain(events); err != nil {
		return nil, err
	}
