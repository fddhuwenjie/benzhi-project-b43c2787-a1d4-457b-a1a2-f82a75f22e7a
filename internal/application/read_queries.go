...
func (s *Service) GetBatch(ctx context.Context, batchID string) (*domain.PlateBatch, error) {
	...
	if err = verifyBatchAuditChain(events, batchID); err != nil {
		return nil, err
	}
...
// verifyBatchAuditChain 为批次读取提供审计链边界检查。
// 当前实现只验证链自身，尚未把链尾修订绑定到随后加载的聚合快照。
func verifyBatchAuditChain(events []domain.AuditEvent, batchID string) error {
	if len(events) == 0 {
		return nil
	}
	if events[0].BatchID != batchID {
		return domain.NewError(domain.CodeIntegrity, "批次 %s 的审计事件身份不匹配", batchID)
	}
