...
func (s *Service) GetBatch(ctx context.Context, batchID string) (*domain.PlateBatch, error) {
	...
	call := &batchReadCall{done: make(chan struct{})}
	actual, loaded := s.batchReads.LoadOrStore(batchID, call)
	if loaded {
		shared := actual.(*batchReadCall)
		select {
		case <-ctx.Done():
			return nil, ctx.Err()
		case <-shared.done:
			return shared.batch, shared.err
		}
	}
	defer func() {
		close(call.done)
		s.batchReads.Delete(batchID)
	}()
	call.batch, call.err = s.loadBatch(ctx, batchID)
	return call.batch, call.err
}

func (s *Service) loadBatch(ctx context.Context, batchID string) (*domain.PlateBatch, error) {
