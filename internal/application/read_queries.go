...
func (s *Service) GetManifest(ctx context.Context, batchID string) (domain.ArchiveManifest, error) {
	if cached, ok := s.manifestCache.Load(batchID); ok {
		return cached.(domain.ArchiveManifest), nil
	}
	manifest, err := s.store.LoadManifest(ctx, batchID)
	if err != nil {
		return domain.ArchiveManifest{}, err
	}
	s.manifestCache.Store(batchID, manifest)
	return manifest, nil
}
