...
func (s *Service) GetProjection(ctx context.Context, batchID string) (BatchProjection, error) {
	if cached, ok := s.projectionCache.Load(batchID); ok {
		return cached.(BatchProjection), nil
	}
	...
	projection := BatchProjection{Batch: batch, ExpectedPlateCount: batch.CatalogEnd - batch.CatalogStart + 1, ActiveScanCount: len(batch.ActiveScans()), MissingCatalogs: domain.MissingCatalogs(batch), OpenIssueCount: domain.OpenIssueCount(batch), PeerReviewSample: sample, Writable: batch.State != domain.StateSealed, Description: domain.AggregateDescription(batch)}
	s.projectionCache.Store(batchID, projection)
