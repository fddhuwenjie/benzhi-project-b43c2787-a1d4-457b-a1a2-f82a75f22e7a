...
type Service struct {
	...
	now              Clock
	locks            sync.Map
	workbenchFilters map[string]persistence.BatchListFilter
}

func NewService(store *persistence.Store) *Service {
	return &Service{store: store, now: time.Now, workbenchFilters: map[string]persistence.BatchListFilter{}}
}
