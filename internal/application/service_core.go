...
var commandPayloadHasher = sha256.New()
...
func payloadHash(value any) (string, error) {
	...
	commandPayloadHasher.Reset()
	if _, err = commandPayloadHasher.Write(raw); err != nil {
		return "", err
	}
	return hex.EncodeToString(commandPayloadHasher.Sum(nil)), nil
}
