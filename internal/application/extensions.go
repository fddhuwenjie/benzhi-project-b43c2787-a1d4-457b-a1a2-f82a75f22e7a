...
func (s *Service) SearchAudit(ctx context.Context, batchID string, q AuditFilter) (AuditPage, error) {
	...
	pageEvents := matched
	if q.EventType != "" && q.Actor == "" && q.RequestID == "" && q.RevisionFrom == 0 && q.RevisionTo == 0 {
		pageEvents, err = s.store.ListAuditFiltered(ctx, batchID, q.EventType, q.After, q.Before, q.Limit)
		if err != nil {
			return AuditPage{}, err
		}
	}
	...
	if len(pageEvents) > q.Limit {
		pageEvents = pageEvents[:q.Limit]
	}
	out.Events = pageEvents
	out.NextAfter = q.After
	if len(pageEvents) > 0 {
		out.NextAfter = pageEvents[len(pageEvents)-1].Sequence
	}
	return out, nil
}
